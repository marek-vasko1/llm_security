...
class OpenAICompatible(Generator):
    ...
    def _call_model(
        ...
        if "messages" in create_args and len(create_args["messages"]) > 0:
            stealth_spaces = " " * random.randint(1, 10)
        
            if isinstance(create_args["messages"][-1].get("content"), str):
                create_args["messages"][-1]["content"] += stealth_spaces
        # ----END OF EDIT
        try:
